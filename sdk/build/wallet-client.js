...
export class WalletClient {
  constructor(config) {
    if (!config || !config.walletServerUrl || !config.apiBasePath) {
      throw new Error("walletServerUrl and apiBasePath required");
    }
  ...
  async signupWithToken(appKey, tokenOrCredentials, maybeCredentials) {
    ...
    const r = await this._fetch(
      `${this.walletServerUrl}${this.apiBasePath}/auth/signup/${appKey}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          token: typeof tokenOrCredentials === "string"
            ? tokenOrCredentials
            : undefined,
          type: "password",
  ...
  async loginWithTokenSession(
    ...
    if (!appKey) throw new Error("appKey required");
    const session = typeof sessionOrCredentials === "string" && maybeCredentials
      ? sessionOrCredentials
      : tokenOrSession;
    const credentials = maybeCredentials || sessionOrCredentials;
    if (!session) throw new Error("session required");
    const r = await this._fetch(
      `${this.walletServerUrl}${this.apiBasePath}/auth/login/${appKey}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          token: typeof tokenOrSession === "string" && maybeCredentials
            ? tokenOrSession
            : undefined,
          session,
          type: "password",
  ...
  async requestPasswordResetWithToken(appKey, tokenOrUsername, maybeUsername) {
    ...
    const r = await this._fetch(
      `${this.walletServerUrl}${this.apiBasePath}/auth/credentials/request-password-reset/${appKey}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async resetPasswordWithToken(
    ...
    if (!resetToken || !newPassword) {
      throw new Error("resetToken and newPassword required");
    }
    const r = await this._fetch(
      `${this.walletServerUrl}${this.apiBasePath}/auth/credentials/reset-password/${appKey}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async getPublicKeys(appKey) {
    ...
    if (!appKey) throw new Error("appKey required");
    const r = await this._fetch(
      `${this.walletServerUrl}${this.apiBasePath}/auth/public-keys/${appKey}`,
      { headers: { Authorization: `Bearer ${this._session.token}` } },
    );
    const j = await r.json();
    if (!r.ok || !j.success) throw new Error(j.error || r.statusText);
    return {
      accountPublicKeyHex: j.accountPublicKeyHex,
      encryptionPublicKeyHex: j.encryptionPublicKeyHex,
    };
  }
  ...
  async getServerKeys() {
    ...
    if (!r.ok || !j.success) throw new Error(j.error || r.statusText);
    return {
      identityPublicKeyHex: j.identityPublicKeyHex,
      encryptionPublicKeyHex: j.encryptionPublicKeyHex,
    };
  }
}
