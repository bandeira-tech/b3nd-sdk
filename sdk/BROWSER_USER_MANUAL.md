...
// Write data
const writeResult = await storage.write("users://current/profile", {
...
if (writeResult.success) {
  console.log("Saved at:", writeResult.record?.ts);
}
...
await db.write("articles://post-1", article);
...
// Write to server
const writeResult = await api.write("users://alice/profile", {
  name: "Alice",
...
if (writeResult.success) {
  console.log("Saved to server at:", writeResult.record?.ts);
} else {
  console.error("Server error:", writeResult.error);
}
...
```
POST   /api/v1/write/{protocol}/{domain}{path}
...
await client.write("data://test", { value: 123 });
```
...
await ws.write("events://user-123/action", {
...
await ws.write("subscriptions://room/chat-123", {
  subscribed: true,
  userId: "user-123",
});
...
try {
  const result = await ws.write("data://test", { value: 1 });
} catch (error) {
...
const result = await hybrid.write("users://alice/profile", {
  name: "Alice",
  email: "alice@example.com",
});

// Succeeds only if BOTH succeed
if (result.success) {
  console.log("Saved locally AND to server");
} else {
  console.error("At least one write failed:", result.error);
}
...
await hybrid.write("articles://post-1", {
  title: "Article",
  content: "...",
  images: ["..."],
});
...
if (result.success) {
  if (result.record?.data.source === 'cache') {
    ...
    // Cache for next time
    await local.write("articles://post-1", result.record?.data);
  }
}

// Write: tries local first, then remote if local fails
await hybrid.write("articles://post-2", {
  title: "New Article",
  content: "...",
});
...
window.addEventListener('online', async () => {
  ...
  for (const item of cached.data) {
    ...
    if (localData.success) {
      await api.write(item.uri, localData.record?.data);
    }
  }
...
async function saveData(uri: string, data: any) {
  return await writeAll.write(uri, data);
}
...
async function loadData(uri: string) {
  ...
  if (result.success) {
    ...
    await memory.write(uri, result.record?.data);
  }
...
async function getCachedData(uri: string, ttlMs: number = 5 * 60 * 1000) {
  ...
  if (fresh.success) {
    // Update cache
    await cache.write(uri, fresh.record?.data);
...
async function optimisticWrite(uri: string, data: any) {
  ...
  const localResult = await local.write(uri, data);

  if (!localResult.success) {
    return { success: false, error: "Local write failed" };
  }
  ...
  const serverResult = await api.write(uri, data);

  if (!serverResult.success) {
    // 4. Rollback local on server error
    console.error("Server write failed, rolling back");
  ...
  setInterval(async () => {
    ...
    if (health.status === "healthy") {
      ...
      for (const item of updates.data) {
        ...
        if (result.success) {
          // Update local cache
          await db.write(item.uri, result.record?.data);
          // Notify callback
...
async function addTodo(text: string) {
  const id = Date.now();
  const result = await storage.write(`todos://user/todo-${id}`, {
    id,
    text,
    completed: false,
    createdAt: Date.now(),
...
async function toggleTodo(id: number) {
  ...
  if (current.success) {
    ...
    await storage.write(uri, updated);
  }
}
...
async function getArticle(slug: string) {
  ...
  if (result.success) {
    ...
    await cache.write(uri, result.record?.data);
...
async function syncArticles() {
  ...
  for (const item of latest.data) {
    ...
    if (article.success) {
      await cache.write(item.uri, article.record?.data);
    }
  }
...
async function updateDocument(docId: string, content: string) {
  ...
  await local.write(uri, doc);

  // Broadcast to other users
  await ws.write(uri, doc);
...
async function loadDocument(docId: string) {
  ...
  if (serverResult.success) {
    // Update local cache
    await local.write(uri, serverResult.record?.data);
...
async function watchDocument(docId: string, onChange: (doc: any) => void) {
  ...
  await ws.write(`subscriptions://${docId}`, { subscribed: true });
  ...
  const interval = setInterval(async () => {
    ...
    if (health.status === "healthy") {
      ...
      if (result.success) {
        // Update local
        await local.write(uri, result.record?.data);
        // Notify
...
All operations return results with a `success` flag. Always check it:

```typescript
const result = await client.write("data://test", { value: 1 });
...
if (!result.success) {
  ...
  console.log("Success:", result.record?.data);
}
```
