 ...
 *
 * Run with: deno task test:e2e:auth
 * Or directly: deno run --allow-net --allow-read --allow-env src/auth/main.ts
 *
...
import { runAuthTests } from "./mod.ts";
import { defaultConfig } from "../core/mod.ts";

async function main() {
  console.log("🔐 Authentication E2E Test Suite");
  ...
  try {
    ...
    console.log("\n✨ Authentication test suite completed successfully!");
    Deno.exit(0);
  } catch (error) {
    console.error("\n❌ Authentication test suite failed with error:");
    console.error(error);
    Deno.exit(1);
  }
}
