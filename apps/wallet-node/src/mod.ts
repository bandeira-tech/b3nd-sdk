...
async function main() {
  ...
  console.log(
    `   Credential Node: ${
      env.get("CREDENTIAL_NODE_URL") || "http://localhost:8842"
    }`,
  );
  console.log(
    `   Proxy Node: ${env.get("PROXY_NODE_URL") || "http://localhost:8842"}`,
  );

  const appBackendUrl = env.get("APP_BACKEND_URL");
  if (appBackendUrl) {
    console.log(
      `   App Backend: ${appBackendUrl}${
        env.get("APP_BACKEND_API_BASE_PATH") || "/api/v1"
      }`,
    );
  } else {
  ...
  console.log(
    `   ✓ Server Identity: ${
      config.serverKeys.identityKey.publicKeyHex.slice(0, 16)
    }...`,
  );
  console.log(
    `   ✓ Server Encryption: ${
      config.serverKeys.encryptionKey.publicKeyHex.slice(0, 16)
    }...`,
  );
  ...
  Deno.serve({
    ...
    onListen: () => {
      ...
      console.log(
        "   GET    /api/v1/proxy/read - Proxy read request (with decryption)",
      );
      console.log(
