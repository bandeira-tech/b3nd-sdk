Feature: Handler as Backend Service
  ...
  in a node's receive pipeline or connected remotely to a Firecat
  node. B3nd composes them; Firecat provides the protocol.

  Scenario: Handler embedded in a custom node
    Given a Firecat node with a vault handler in its receive pipeline
  ...
  Scenario: Handler connected remotely via polling
    Given a handler process connected to a remote Firecat node
