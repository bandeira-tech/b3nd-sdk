...
(cd "$ROOT_DIR/installations/http-server" && \
  BACKEND_URL="$BACKEND_URL" \
  SCHEMA_MODULE=./example-schema.ts \
  PORT=9942 \
  CORS_ORIGIN="*" \
...
if $FULL; then
  ...
  if [ ! -f "$ROOT_DIR/installations/wallet-server/.env" ]; then
    echo -e "${YELLOW}⚠ Wallet server .env not found. Skipping wallet & app servers.${NC}"
    echo -e "  Run: cd installations/wallet-server && deno run -A scripts/generate-keys.ts"
    FULL=false
  else
    echo -e "${GREEN}▶ Starting Wallet Server (port 9943)...${NC}"
    (cd "$ROOT_DIR/installations/wallet-server" && \
      PORT=9943 \
      CREDENTIAL_NODE_URL=http://localhost:9942 \
      PROXY_NODE_URL=http://localhost:9942 \
    ...
    (cd "$ROOT_DIR/installations/app-backend" && \
      PORT=9944 \
      DATA_NODE_URL=http://localhost:9942 \
...
(cd "$SCRIPT_DIR/app" && npm run dev 2>&1 | sed 's/^/  [frontend] /') &
