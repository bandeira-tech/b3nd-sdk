...
Deno.test("parallelBroadcast - writes to all backends", async () => {
  const a = new FirecatDataClient(new MemoryStore());
  const b = new FirecatDataClient(new MemoryStore());
...
Deno.test("firstMatchSequence - reads from first backend that has data", async () => {
  const primary = new FirecatDataClient(new MemoryStore());
  const fallback = new FirecatDataClient(new MemoryStore());
