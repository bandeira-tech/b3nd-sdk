...
import { runSharedSuite } from "../../testing/shared-suite.ts";
...
const postgresSetupPromise: Promise<PostgresSetupResult> = (async () => {
  ...
  if (await isReady()) {
    console.log(`[postgres-test] Reusing running container "${containerName}"`);
  } else {
    ...
    if (runResult.code !== 0) {
      throw new Error(
        "Failed to start PostgreSQL Docker container for tests",
      );
    }
    ...
    if (!(await isReady())) {
      throw new Error(
        "PostgreSQL Docker container did not become ready in time",
      );
    }
  }
