...
const cmd = new Deno.Command("docker", {
  args: [
    ...
    "BACKEND_URL=memory://",
    "-e",
    `OPERATOR_KEY=${operator.publicKeyHex}`,
    "-e",
    `NODE_PRIVATE_KEY_PEM=${nodePem}`,
    "-e",
    `NODE_ENCRYPTION_PRIVATE_KEY_HEX=${nodeEncHex}`,
    "-e",
    `OPERATOR_ENCRYPTION_PUBLIC_KEY_HEX=${opEnc.publicKeyHex}`,
    image,
  ],
  stdin: "inherit",
  stdout: "inherit",
